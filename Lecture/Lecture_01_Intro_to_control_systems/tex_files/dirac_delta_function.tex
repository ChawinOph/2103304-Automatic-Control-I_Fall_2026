\centering
\begin{tikzpicture}
    \begin{axis}[
        % Force markers to draw on top of grids/axis lines
        set layers, mark layer=axis foreground,
        width=\linewidth, height=5cm,
        xlabel={$t$}, ylabel={$\delta(t)$},
        xmin=-0.25, xmax=0.25, ymin=-0.25, ymax=1.25,
        xtick={0}, ytick={1}, yticklabels={$1$},
        axis lines=center,
        every axis x label/.style={at={(current axis.right of origin)},anchor=south west},
        every axis y label/.style={at={(current axis.above origin)},anchor=south},
        clip=false,
    ]
        % arrow of "unit area" at t=0 (standard impulse convention)
        \addplot[very thick, blue, -{Stealth[length=3mm]}] coordinates {(0,0) (0,1)};
        % baseline
        \addplot[very thick, blue] coordinates {(-0.15,0) (0.15,0)};
        % \addplot[only marks, mark=o, blue, fill=white] coordinates {(0,0)};
        \node[blue, anchor=south west] at (axis cs:0.05,0.95) {\small (area $=1$)};
    \end{axis}
\end{tikzpicture}