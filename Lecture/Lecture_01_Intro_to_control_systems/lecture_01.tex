...
\begin{frame}{Linear System: Additivity}
    ...
    % \centering
    \vspace{1em}
    \emph{Additivity:} the response to a sum of inputs is the sum of the individual responses.
    \[
...
\begin{frame}{Time-Invariant System}
    ...
    % \centering
    \vspace{1em}
    \emph{Time Invariance (TI):} the system behaves the same at any moment. Shifting the input in time by $t_0$ shifts the output by the same $t_0$, with its shape unchanged:
    \[
...
\begin{frame}{Linear Time-Invariant System (LTI)}
    ...
    % \centering
    \vspace{1em}
    An \emph{LTI} system is \emph{linear} (homogeneity $+$ additivity) and \emph{time-invariant}. Hence a weighted sum of shifted impulses produces the same weighted sum of shifted impulse responses:
    \[
