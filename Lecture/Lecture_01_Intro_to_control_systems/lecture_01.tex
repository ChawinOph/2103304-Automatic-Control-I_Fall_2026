...
% \large
    \begin{itemize}
        ...
        \item Class Schedule: Monday, 13:00 - 16:00
        \item Location: Room 409, Building 3, Faculty of Engineering
        \item Instructor: Chawin Ophaswongse, Ph.D. % (CWO) \\
        \item Co-Instructor: Assoc. Prof. Thitima Jintanawan, Ph.D. %(TJW)
        \item TAs: Namo (Discord Admin)
        \item Office Hours: TBD at Hans Buntli Building, 4th Floor, Room 407
    \end{itemize}
\end{frame}
...
\begin{frame}{Sifting Property of Impulse Function}
    ...
    \[
        \text{Since } f(t)\,\delta(t-t_0) = f(t_0)\,\delta(t-t_0), \quad \int_{-\infty}^{\infty}\!\! f(t)\,\delta(t-t_0)\,dt = f(t_0).
    \]
    \pause
    % \vspace{0.5em}
    \begin{columns}[t]
        ...
        \begin{column}{0.5\textwidth}
            ...
            \begin{tikzpicture}
                \begin{axis}[
                    ...
                    xmin=0, xmax=3.5, ymin=-1.5, ymax=1.5,
                    xtick={0.375}, xticklabels={$t_0$}, ytick={1},
                    axis x line=middle, axis y line=middle,
                    every axis x label/.style={at={(current axis.right of origin)},anchor=north west},
                    every axis y label/.style={at={(current axis.above origin)},anchor=south},
                    clip=false,
                ]
                    \addplot[very thick, red, smooth, samples=200, domain=0:3.5]{sin(deg(2*pi*x))};
                    \addplot[very thick, blue, -{Stealth[length=2.5mm]}] coordinates {(0.375,0) (0.375,1)};
                    \node[blue, anchor=south west] at (axis cs:0.375,0.85) {\small $\delta(t-0.375)$};
                \end{axis}
            \end{tikzpicture}
            % \vspace{0.5em}
            \[
                \int_{-\infty}^{\infty}\!\! \sin(2\,\pi\,t)\,\delta(t-0.375)\,dt = \sin\tfrac{3\pi}{4}
            \]
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Decomposition Property of Impulse Function}
    From sifting property which we only evaluate the integral where the impulse occure at $t_0$
    \[
        \int_{-\infty}^{\infty}\!\! f(t_0)\,\delta(t-t_0)\,dt = f(t_0).
    \]
    If we want to evaluate the function $f(t)$ through this integral, change 
