...
\begin{frame}{Non-continuous Forcing Function}
    \begin{columns}
        \begin{column}{0.5\textwidth}
            How do we solve ODEs when the forcing function is a rectangular pulse?
            % \[
            %     u(t-1) - u(t-2)
            % \]
            % \begin{itemize}
...
\begin{frame}
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \addplot+[ycomb, samples=12, domain=0:10, thick, mark=*] 
            {cos(deg(x * pi / 4))}; 
...
\begin{frame}
    ...
    $$ \mathcal{L}\{f(t) * g(t)\} = \int_{0}^{\infty} f(\tau)g(t-\tau) d\tau = F(s)G(s)$$
\end{frame}
